...
public class ExaminateurResourceIntTest {
    ...
    private static final String DEFAULT_TELEPHONE = "724107964819445";
    private static final String UPDATED_TELEPHONE = "23741636761253";
     ...
     *
     * @see dz.talcorp.ae.repository.search.ExaminateurSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createExaminateur() throws Exception {
        ...
        // Validate the Examinateur in Elasticsearch
        verify(mockExaminateurSearchRepository, times(1)).save(testExaminateur);
    }
    ...
    public void createExaminateurWithExistingId() throws Exception {
        ...
        // Validate the Examinateur in Elasticsearch
        verify(mockExaminateurSearchRepository, times(0)).save(examinateur);
    }
    ...
    public void deleteExaminateur() throws Exception {
        ...
        // Validate the Examinateur in Elasticsearch
        verify(mockExaminateurSearchRepository, times(1)).deleteById(examinateur.getId());
    }

    @Test
    @Transactional
    public void searchExaminateur() throws Exception {
        // Initialize the database
        examinateurRepository.saveAndFlush(examinateur);
        when(mockExaminateurSearchRepository.search(queryStringQuery("id:" + examinateur.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(examinateur), PageRequest.of(0, 1), 1));
        // Search the examinateur
        restExaminateurMockMvc.perform(get("/api/_search/examinateurs?query=id:" + examinateur.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(examinateur.getId().intValue())))
