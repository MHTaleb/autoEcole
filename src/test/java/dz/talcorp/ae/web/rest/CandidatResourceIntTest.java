...
public class CandidatResourceIntTest {
    ...
    private static final String DEFAULT_TELEPHONE = "3154173752040131";
    private static final String UPDATED_TELEPHONE = "4229229506237";
     ...
     *
     * @see dz.talcorp.ae.repository.search.CandidatSearchRepositoryMockConfiguration
     */
    @Autowired
    private CandidatSearchRepository mockCandidatSearchRepository;
    ...
    public void createCandidatWithExistingId() throws Exception {
        ...
        // Validate the Candidat in Elasticsearch
        verify(mockCandidatSearchRepository, times(0)).save(candidat);
    }
    ...
    public void deleteCandidat() throws Exception {
        ...
        // Validate the Candidat in Elasticsearch
        verify(mockCandidatSearchRepository, times(1)).deleteById(candidat.getId());
    }

    @Test
    @Transactional
    public void searchCandidat() throws Exception {
        // Initialize the database
        candidatRepository.saveAndFlush(candidat);
        when(mockCandidatSearchRepository.search(queryStringQuery("id:" + candidat.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(candidat), PageRequest.of(0, 1), 1));
        // Search the candidat
        restCandidatMockMvc.perform(get("/api/_search/candidats?query=id:" + candidat.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(candidat.getId().intValue())))
            .andExpect(jsonPath("$.[*].photoContentType").value(hasItem(DEFAULT_PHOTO_CONTENT_TYPE)))
            ...
            .andExpect(jsonPath("$.[*].pere").value(hasItem(DEFAULT_PERE)))
