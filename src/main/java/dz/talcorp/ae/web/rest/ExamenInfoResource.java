...
public class ExamenInfoResource {
    ...
    /**
     * SEARCH  /_search/examen-infos?query=:query : search for the examenInfo corresponding
     * to the query.
     *
     * @param query the query of the examenInfo search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/examen-infos")
    public ResponseEntity<List<ExamenInfoDTO>> searchExamenInfos(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ExamenInfos for query {}", query);
        Page<ExamenInfoDTO> page = examenInfoService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/examen-infos");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
