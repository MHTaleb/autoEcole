...
public class EntraineurResource {
    ...
    /**
     * SEARCH  /_search/entraineurs?query=:query : search for the entraineur corresponding
     * to the query.
     *
     * @param query the query of the entraineur search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/entraineurs")
    public ResponseEntity<List<EntraineurDTO>> searchEntraineurs(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Entraineurs for query {}", query);
        Page<EntraineurDTO> page = entraineurService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/entraineurs");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
