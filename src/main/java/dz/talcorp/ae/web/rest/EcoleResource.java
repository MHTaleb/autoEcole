...
public class EcoleResource {
    ...
    public ResponseEntity<List<EcoleDTO>> searchEcoles(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Ecoles for query {}", query);
        Page<EcoleDTO> page = ecoleService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/ecoles");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
